...
def calc_ngram_counts_freqs(text, n):
    count_ht = dict() 
...
def author_ngrams_count(author_frag_pair, n):
    fragments = author_frag_pair[1]
    return (author_frag_pair[0], map(lambda x : calc_ngram_counts_freqs(x, n), fragments)) # (author_str, (count_ht, freq_ht))

def count_all_authors(fragmentation, ngram_len):
    return { author_dir : author_ngrams_count(author_frag_pair, ngram_len) for author_dir, author_frag_pair in fragmentation.items() }

def get_top_k_ngrams(author_ht_tuples, top_k):
    total_count_dict = {}
    for author_ht_tuple in author_ht_tuples:
        join_dicts(total_count_dict, author_ht_tuple[1][0], operator.add)
